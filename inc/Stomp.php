...
class Stomp
{
    ...
    protected $_read_timeout_seconds = 10;
    ...
    protected function _makeConnection ()
    {
        ...
        while (! $connected && $att ++ < $this->_attempts) {
            ...
            if (!is_resource($this->_socket) && $att >= $this->_attempts && !array_key_exists($i + 1, $this->_hosts)) {
                ...
                $connected = true;
                $this->_currentHost = $i;
				stream_set_timeout($this->_socket, $this->_read_timeout_seconds, $this->_read_timeout_microseconds/1000);
                break;
            }
        }
        if (! $connected) {
     ...
     */
    public function setReadTimeout($seconds, $us = 0) 
    {
        $this->_read_timeout_seconds = $seconds;
        $this->_read_timeout_microseconds = $us;
    }
    ...
    public function readFrame ($block = false) {
        ...
        $end = false;
	   	...
	   	$timeout_sec = $block? null : $this->_read_timeout_seconds;
		$timeout_usec = $block? null : $this->_read_timeout_microseconds;
		
		stream_set_blocking($this->_socket, 0);

		while(($nullpos = strpos($this->_rcvbuf, "\x00")) === false) {
			$read_s = array($this->_socket);
			$write_s = null;
			$exp_s = null;
			stream_select($read_s, $write_s, $exp_s, $timeout_sec, $timeout_usec);
		...
		$data = substr($this->_rcvbuf, 0, $nullpos);
		$this->_rcvbuf = substr($this->_rcvbuf, $nullpos+1);
    ...
    {
		if (strpos($this->_rcvbuf, "\x00") !== false) {
			return true;
		}
