...
class Facebook
{
	...
	public static function get_username($token, $secret = null) {
		$f = self::connect();
		try {
			$me = $f->api('/me', 'GET', array('access_token' => $token));
			return array(
				'name' => $me['name'],
				'user_id' => $me['id'],
			);
	...
	public static function get_local_friends() {
		$f = self::connect();
		...
		try {
			$friends = $f->api('/me/friends', 'GET', array('access_token' => $token));
			$fb_uids = get_ids($friends['data'], 'id');
			$local_uids = array_values(m('oauth')->get_local_uid_by_remote_uid_mass('facebook', $fb_uids));
			
			$leaders = m('user')->get_leaders_of(stash()->user_id);

			$new_uids = array_diff($local_uids, $leaders);
