...
class HTML_BBCodeParser_Filter_Links extends HTML_BBCodeParser_Filter
{
    ...
    function smarterPPLinkExpand($matches)
    {
        ...
        // huf's fix for cases where we have:
        // [url=http://somesite]http://somesite[/url]
        // that is, the properly formatted url tag contains a plain-text url
        // without this, we'd get a broken autolinked crap
        $re = "/^\\{$o}url=http[^]]*\\{$c}\$/";
        if (preg_match($re, $matches[1])) {
            return $matches[0];
        }
